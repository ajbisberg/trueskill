...
namespace trueskill
{
    ...
    public class ChessAnalysis
    {
        ...
        public static void Run()
        {
            ...
            // foreach(var y in whiteData)
            // {
            //     Console.WriteLine("Length of Years",y.Length);
            //     foreach(var p in y) { 
            ...
            Console.WriteLine("white advantage = {0} (truth = {1}) \n\n", engine.Infer<Gaussian>(whiteAdvantage), parameters.whiteAdvantage);
            int countPrinted = 0;
            for (int y = 0; y < nYears; y++)
            {
                for (int p = 0; p < nPlayers; p++)
                {
                    if (y >= firstYear.ObservedValue[p])
                    {
                        ...
                        Console.WriteLine("drawMargin[{0}][{1}] = {2} (truth = {3:g4}) \n\n", y, p, drawMarginPost[y][p], parameters.drawMargin[y][p]);
                    }
                }
            }
        }
        ...
        public static void GenerateData(Parameters parameters, int[] firstYear, out int[][] whiteData, out int[][] blackData, out int[][] outcomeData)
        {
            ...
            Console.WriteLine("Years: {0}",nYears);
            Console.WriteLine("Players: {0}",nPlayers);
...
//         public static void RunTrueSkill() {
//             int mean1Skill = 1500;  int mean2Skill = 1600;
//             Variable<double> team1Skill = Variable.GaussianFromMeanAndVariance(mean1Skill, Math.Pow(100,2));
//             Variable<double> team2Skill = Variable.GaussianFromMeanAndVariance(mean2Skill, Math.Pow(100,2));
            
//             Variable<double> team1Perf = Variable.GaussianFromMeanAndVariance(team1Skill, Math.Pow(5,2));
//             Variable<double> team2Perf = Variable.GaussianFromMeanAndVariance(team2Skill, Math.Pow(5,2));

//             Variable.ConstrainTrue(team1Perf > team2Perf);
//             var player = new Range(2);
            
//             // var perfs = Variable.Array<double>(player);
//             // perfs[0] = team1Perf;
//             // perfs[1] = team2Perf;
//             // var games = new Range(1);
//             // var t1Perfs = Variable.Array<double>(games);
//             // t1Perfs[0] = team1Perf;

//             // var t2Perfs =  Variable.Array<double>(games);
//             // t2Perfs[0] = team2Perf;

//             // By default this uses EP
//             InferenceEngine ie = new InferenceEngine(); 

//             Gaussian avgTeam1Perf = ie.Infer<Gaussian>(team1Perf);
//             Gaussian avgTeam2Perf = ie.Infer<Gaussian>(team2Perf);
//             Console.WriteLine(avgTeam1Perf);
//             Console.WriteLine(avgTeam2Perf);
//             // Gaussian avgTeam1Perf = ie.Infer<Gaussian>(team1Perf);
//             // Gaussian avgTeam2Perf = ie.Infer<Gaussian>(team2Perf);

//             // List<Gaussian> li = new List<Gaussian>();
//             // li.Add(avgTeam1Perf);
//             // li.Add(avgTeam2Perf);
                        
//             // double prob = 1.0;
//             // Variable<double> t1Wins = Variable.Bernoulli(prob)
//         }

//         // public Bernoulli InferSkillGreaterThan(double skill1) {
//         //     return InferenceEngine.Infer<Bernoulli>(skill1 > skill2);
//         // }
