...
# ts.setup(mu=1500,sigma=100,draw_probability=0)
# print('ts env is: ',ts.global_env())
...
# tourneys = []
# print(
#     rating_i.loc['eUnited'].rating.mu,
#     rating_i.loc['OpTic Gaming'].rating.mu
# )
# prob = ts_win_probability(
#     [rating_i.loc['eUnited'].rating],
#     [rating_i.loc['OpTic Gaming'].rating],
#     ts
# )
# print('eu beats optic ',prob,' of the time')
...
for i,td in enumerate(season.tourneys):
    ...
    # t.set_first_series_rating(rating_i,ts)
    # t.play(ts,acc=True)
    # print(t.rating_f)
    tourneys.append(t)
t1 = perf_counter()
...
def validate(tourneys,mu,sigma,beta,tau,do_test=False):
    ...
    ts.setup(mu=mu,sigma=sigma,draw_probability=0,beta=beta,tau=tau)
    print('ts env is:',ts.global_env())
    rating_i = initialize_rating(Tournament(TournamentData('CWL Champs 2017',data_path).data).raw_data,ts)
    ...
    for i in range(len(tourneys)-2):
        ...
        test = tourneys[i+2]
        for train in trains:
            train.set_first_series_rating(rating_i,ts)
            train.play(ts)
            rating_f = train.final_ratings()
            rating_i = rating_f #regressfunc(merge_elo(elo_i,elo_f))
            tr_len += len(train.tournament)
        
        validate.set_first_series_rating(rating_i,ts)
        validate.play(ts,acc=True)
...
cv_metric_frame.to_csv('cv_results_ts.csv',index=False)
